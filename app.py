import streamlit as st
import pandas as pd
import plotly.express as px


# --- Configuração da Página ---
# Define o título da página, o ícone e o layout para ocupar a largura inteira.
st.set_page_config(
    page_title="Dashboard de Salários na Área de Dados",
    page_icon="📊",
    layout="wide"
)

df = pd.read_csv("final_data.csv")

st.sidebar.header("Filtros")

# Filtro anos
anos_disponiveis = sorted(df['work_year'].unique())
anos_selecionados = st.sidebar.multiselect("Ano", anos_disponiveis, default=anos_disponiveis)

# Filtro Senioridade
senioridade_disponiveis = sorted(df['experience_level_pt'].unique())
senioridades_selecionadas = st.sidebar.multiselect("Senioridade", senioridade_disponiveis, default=senioridade_disponiveis)

# Filtro Tipo de Contrato
contrato_disponiveis = sorted(df['employment_type_pt'].unique())
contratos_selecionados = st.sidebar.multiselect("Tipo de Contrato", contrato_disponiveis, default=contrato_disponiveis)

# Filtro Tamanho da Empresa
tamanhos_disponiveis = sorted(df['company_size'].unique())
tamanhos_selecionados = st.sidebar.multiselect("Tamanho da Empresa", tamanhos_disponiveis, default=tamanhos_disponiveis)

# --- Filtragem no DataFrame ---
df_filtrado = df[
    (df["work_year"].isin(anos_selecionados)) &
    (df["experience_level_pt"].isin(senioridades_selecionadas)) &
    (df["employment_type_pt"].isin(contratos_selecionados)) &
    (df["company_size"].isin(tamanhos_selecionados))   
]

st.title("Dashboard de Salários na Área de Dados")
st.markdown("Explore os dados salariais da área de dados nos últimos anos.")

st.subheader("Métricas gerais (Salário anual em USD)")

if not df_filtrado.empty:
    salario_medio = df_filtrado['salary_in_usd'].mean()
    salario_mediano = df
    salario_maximo = df_filtrado['salary_in_usd'].max()
    total_registros = df_filtrado.shape[0]
    cargo_mais_frequente = df_filtrado["job_title"].mode()[0]
else:
    salario_medio, salario_maximo, total_registros, cargo_mais_frequente = 0, 0, 0, ""
    
col1, col2, col3, col4 = st.columns(4)

col1.metric("Salário Médio", f"${salario_medio:.0f}")
col2.metric("Salário Máximo", f"${salario_maximo:.0f}")
col3.metric("Total de Registros", total_registros)
col4.metric("Cargo Mais Frequente", cargo_mais_frequente)

st.markdown("---")

st.subheader("Gráficos")

col_graf1, col_graf2 = st.columns(2)

with col_graf1:
    if not df_filtrado.empty:
        top_cargos = df_filtrado.groupby("job_title")["salary_in_usd"].mean().nlargest(10).sort_values(ascending=True).reset_index()
        grafico_cargos = px.bar(
            top_cargos,
            x="salary_in_usd",
            y="job_title",
            orientation="h",
            title="Top 10 Cargos por Salário Médio",
            labels={"salary_in_usd": "Salário Médio", "job_title": "Cargo"}
        )
        grafico_cargos.update_layout(title_x=0.1, yaxis={'categoryorder':'total ascending'})
        st.plotly_chart(grafico_cargos, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para o filtro selecionado.")

with col_graf2:
    if not df_filtrado.empty:
        grafico_hist = px.histogram(
            df_filtrado,
            x="salary_in_usd",
            nbins=30,
            title="Distribuição Salarial anual",
            labels={"salary_in_usd": "Faixa salarial (USD)", "count": ''}
        )
        grafico_hist.update_layout(title_x=0.1)
        st.plotly_chart(grafico_hist, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para o filtro selecionado.")

col_graf3, col_graf4 = st.columns(2)

with col_graf3:
    if not df_filtrado.empty:
        remoto_contagem = df_filtrado["remote_ratio_pt"].value_counts().reset_index()
        remoto_contagem.columns = ["tipo_trabalho", "quantidade"]
        grafico_remoto = px.pie(
            remoto_contagem,
            names="tipo_trabalho",
            values="quantidade",
            title="Proporção dos tipos de trabalho",
            hole=0.5,
            labels={"tipo_trabalho": "Modalidade", "quantidade": "Quantidade"}
            )
        grafico_remoto.update_traces(textinfo='percent+label')
        grafico_remoto.update_layout(title_x=0.1)
        st.plotly_chart(grafico_remoto, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para o filtro selecionado.")

with col_graf4:
    if not df_filtrado.empty:
       df_ds = df_filtrado[df_filtrado["job_title"] == "Data Scientist"]
       df_ds = df_filtrado[df_filtrado['job_title'] == 'Data Scientist']
       mean_ds_per_country = df_ds.groupby('country_iso3')['salary_in_usd'].mean().reset_index()
       grafico_paises = px.choropleth(
            mean_ds_per_country,
            locations='country_iso3',
            color='salary_in_usd',
            color_continuous_scale='rdylgn',
            title='Média de Salário de Cientistas de Dados por País',
            labels={'salary_in_usd': 'Salário médio (USD)', 'country_iso3': 'País'}
            )
       grafico_paises.update_layout(title_x=0.1)
       st.plotly_chart(grafico_paises, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para o filtro selecionado.")


st.subheader("Dados Detalhados")
st.dataframe(df_filtrado)
st.markdown("---")